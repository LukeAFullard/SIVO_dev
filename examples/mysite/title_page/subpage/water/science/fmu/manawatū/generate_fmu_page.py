...
for icon in icons:
    ...
    sites = 0
    if key == "Ammoniacal-N":
        sites = state_summary["FMUs"]["Manawatū"].get("Ammoniacal-N", {}).get("Total Sites for Attribute", 0) + \
                state_summary["FMUs"]["Manawatū"].get("Nitrate-N", {}).get("Total Sites for Attribute", 0)
    elif key:
        fmu_obj = state_summary["FMUs"].get("Manawatū", {})
