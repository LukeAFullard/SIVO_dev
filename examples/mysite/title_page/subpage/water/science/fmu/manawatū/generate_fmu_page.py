...
for folder, p_info in parameters_state.items():
    ...
    fmu_data = state_summary["FMUs"]["Manawatū"].get(p_info["json_key"])
...
if os.path.exists(nitrogen_template_path):
    ...
    ammo_fmu_data = state_summary["FMUs"]["Manawatū"].get("Ammoniacal-N")
    ammo_region_data = state_summary["Region"]["Ammoniacal-N"]
    nitrate_fmu_data = state_summary["FMUs"]["Manawatū"].get("Nitrate-N")
...
icons = [
    {"id": "icon1", "img": "../../../../../assets/water/20221123_OrangaWai_IconEcoli.png", "hover": "E. <i>coli</i>", "md_dir": "md/ecoli", "map_file": "results/ecoli_map.html", "param": "ecoli"},
    ...
    {"id": "icon4", "img": "../../../../../assets/water/20221123_OrangaWai_IconP.png", "hover": "Phosphorus", "md_dir": "md/Phosphorus", "map_file": "results/phosphorus_map.html", "param": "Phosphorus"},
    {"id": "icon5", "img": "../../../../../assets/water/20221123_OrangaWai_IconChlA.png", "hover": "Algae", "md_dir": "md/Algae", "map_file": "results/algae_map.html", "param": "Algae"},
    {"id": "icon6", "img": "../../../../../assets/water/20221123_OrangaWai_IconAquaticLife.png", "hover": "Invertebrates", "md_dir": "md/inverts", "map_file": "results/inverts_map.html", "param": "inverts"}
]

hidden_icons = []
for icon in icons:
    if icon["param"] == "Nitrogen":
        ammo_sites = state_summary["FMUs"]["Manawatū"].get("Ammoniacal-N", {}).get("Total Sites for Attribute", 0)
        nitrate_sites = state_summary["FMUs"]["Manawatū"].get("Nitrate-N", {}).get("Total Sites for Attribute", 0)
        if ammo_sites == 0 and nitrate_sites == 0:
            hidden_icons.append(icon["id"])
    else:
        p_info = parameters_state.get(icon["param"])
        if p_info:
            fmu_sites = state_summary["FMUs"]["Manawatū"].get(p_info["json_key"], {}).get("Total Sites for Attribute", 0)
            if fmu_sites == 0:
                hidden_icons.append(icon["id"])
...
for folder, json_key in parameters_trend.items():
    fmu_data = trend_data['FMUs']['Manawatū'].get(json_key)
