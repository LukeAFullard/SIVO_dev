...
for folder, p_info in parameters_state.items():
    ...
    fmu_data = state_summary["FMUs"]["Waiopehu"].get(p_info["json_key"])
...
if os.path.exists(nitrogen_template_path):
    ...
    ammo_fmu_data = state_summary["FMUs"]["Waiopehu"].get("Ammoniacal-N")
    ammo_region_data = state_summary["Region"]["Ammoniacal-N"]
    nitrate_fmu_data = state_summary["FMUs"]["Waiopehu"].get("Nitrate-N")
...
for icon in icons:
    if icon["param"] == "Nitrogen":
        ammo_sites = state_summary["FMUs"]["Waiopehu"].get("Ammoniacal-N", {}).get("Total Sites for Attribute", 0)
        nitrate_sites = state_summary["FMUs"]["Waiopehu"].get("Nitrate-N", {}).get("Total Sites for Attribute", 0)
        if ammo_sites == 0 and nitrate_sites == 0:
            hidden_icons.append(icon["id"])
    else:
        p_info = parameters_state.get(icon["param"])
        if p_info:
            fmu_sites = state_summary["FMUs"]["Waiopehu"].get(p_info["json_key"], {}).get("Total Sites for Attribute", 0)
            if fmu_sites == 0:
                hidden_icons.append(icon["id"])
...
for folder, json_key in parameters_trend.items():
    fmu_data = trend_data['FMUs']['Waiopehu'].get(json_key)
...
ammo_fmu_data = trend_data['FMUs']['Waiopehu']["Ammoniacal Nitrogen (NH4)"]
ammo_region_data = trend_data['Region']["Ammoniacal Nitrogen (NH4)"]
nitrate_fmu_data = trend_data['FMUs']['Waiopehu']["Nitrate Nitrogen (NO3)"]
