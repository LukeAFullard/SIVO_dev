...
ammo_fmu_data = state_summary["FMUs"]["Waiopehu"]["Ammoniacal-N"]
ammo_region_data = state_summary["Region"]["Ammoniacal-N"]
nitrate_fmu_data = state_summary["FMUs"]["Waiopehu"]["Nitrate-N"]
...
for icon in icons:
    ...
    sites = 0
    if key == "Ammoniacal-N":
        sites = state_summary["FMUs"]["Waiopehu"].get("Ammoniacal-N", {}).get("Total Sites for Attribute", 0) + \
                state_summary["FMUs"]["Waiopehu"].get("Nitrate-N", {}).get("Total Sites for Attribute", 0)
    elif key:
        fmu_obj = state_summary["FMUs"].get("Waiopehu", {})
